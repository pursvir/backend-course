...
async def test_auth_flow(
    email: str, password: str, register_status_code: int, login_status_code: int,
    ac: AsyncClient
):
    ...
    if response.status_code == 200:
        ...
        assert response.cookies["access_token"] # Можно было ещё проверить содержимое этого токена, но мне лень)
